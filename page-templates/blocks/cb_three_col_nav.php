        ...
        <div class="row">
            <div class="col-lg-4 mb-4 three_col_nav__card">
                <div class="three_col_nav__inner">
                    <img src="<?=get_stylesheet_directory_uri()?>/img/icon--applications.svg"
                        alt="" class="icon">
                    <h2 class="three_col_nav__title">
                        <?=__('Industries', 'cb-hydronix')?>
                    </h2>
                    <div class="three_col_nav__content">
                        <?=get_field('app_content')?>
                    </div>
                </div>
                <select class="form-select mb-4" name="" id="byApplication">
                    <option value="">
                        <?=__('Select Industry', 'cb-hydronix')?>
                    </option>
                    <?php
...
?>
            ...
            <div class="col-lg-4 mb-4 three_col_nav__card">
                <div class="three_col_nav__inner">
                    <img src="<?=get_stylesheet_directory_uri()?>/img/icon--benefits.svg"
                        alt="" class="icon">
                    <h2 class="three_col_nav__title">
                        <?=__('Benefits', 'cb-hydronix')?>
                    </h2>
                    <div class="three_col_nav__content">
                        <?=get_field('ben_content')?>
                    </div>
                </div>
                <div class="mb-4">
                    <a class="btn btn--orange mx-auto w-100"
                        href="<?=__('/moisture-measurement/', 'cb-hydronix')?>"><?=__('Read more', 'cb-hydronix')?></a>
                </div>
            </div>
            <div class="col-lg-4 mb-4 three_col_nav__card">
                <div class="three_col_nav__inner">
                    <img src="<?=get_stylesheet_directory_uri()?>/img/icon--process-control.svg"
                        alt="" class="icon">
                    <h2 class="three_col_nav__title">
                        <?=__('Process control', 'cb-hydronix')?>
                    </h2>
                    <div class="three_col_nav__content">
                        <?=get_field('pc_content')?>
                    </div>
                </div>
                <select class="form-select mb-4" name="" id="byProcess">
                    <option value="">
                        <?=__('Select Process', 'cb-hydronix')?>
                    </option>
                    <?php
        ...
        $('#byApplication').on('change', function() {
            ...
            if (page ===
                ...
                window.location.href =
                    "<?=$lang?>/<?=__('measure', 'cb-hydronix')?>/" +
                    page + "/";
            } else if (page ===
                '<?=__('organic', 'cb-hydronix')?>'
                ) {
                window.location.href =
                    "<?=$lang?>/<?=__('measure', 'cb-hydronix')?>/<?=__('grain-bulk-solids', 'cb-hydronix')?>/";
            } else {
                window.location.href =
                    "<?=$lang?>/<?=__('industries', 'cb-hydronix')?>/" +
                    page + "/";
            }
        });
