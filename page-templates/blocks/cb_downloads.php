...
<section class="downloads py-5">
    <div class="container-xl">
        <div class="row mb-4">
            <div class="col-md-3">
                <label for="dsub" class="form-label"><?=__('Product','cb-hydronix')?>:</label>
            ...
            <div class="col-md-3">
                <label for="dind" class="form-label"><?=__('Industry','cb-hydronix')?>:</label>
                <select name="dind" id="dind" data-field-name="dind" class="form-select mb-4 filter">
                    <option value=""><?=__('Select all','cb-hydronix')?></option>
                    <?php
                    foreach ($dinds as $dt) {
                        // cbdump($dt);
                        if ($dt->slug == 'concrete-construction' || $dt->slug == 'organic') {
                            continue;
                        }
                        echo '<option value="' . $dt->slug . '">' . $dt->name . '</option>';
                    }
                    echo '<option value="" disabled>-- ' . __('General','cb-hydronix') . ' --</option>';
                    echo '<option value="concrete-construction">' . __('Concrete & Construction','cb-hydronix') . '</option>';
                    echo '<option value="organic">' . __('Organic','cb-hydronix') . '</option>';

                    ?>
                </select>
            </div>
            <div class="col-md-3">
                <label for="dtype" class="form-label"><?=__('Type','cb-hydronix')?>:</label>
            ...
            <div class="col-md-3">
                <label for="dlang" class="form-label"><?=__('Language','cb-hydronix')?>:</label>
            ...
            <div class="col-md-9">
                <label for="dtext" class="form-label"><?=__('Search Text','cb-hydronix')?>:</label><input type="text" class="form-control filter" id="dtext">
            </div>
            <div class="col-md-3 d-flex justify-content-end">
                <button id="submit" class="btn btn-primary me-2 mb-0 mt-3 btn--small">
                    ...
                    <span id="submitText"><?=__('Search','cb-hydronix')?></span>
                </button>
...
$("#submit").click(function(){
    let endpoint = '/wp-content/themes/cb-hydronix/attachments.php'
    $("#hiddenDSub").empty();
    $("#hiddenDType").empty();
    $("#hiddenDLang").empty();
    $("#hiddenDInd").empty();
...
*/
    ...
    $.ajax({
        url: endpoint + "?dsub=" + selDSub + "&dtype=" + selDType + "&dlang=" + selDLang + "&dtext=" + selDText + "&dind=" + selDInd,
